...
設計判断の根拠は保つ。「task の public IP が起動ごとに変わっても DB へ到達できる」という結論は残したうえで、DB 側の防御構成に関する記述を落とす。

> **注記**: この線引きは後に `discussion.md` の論点8 で覆った。機密情報の標準ができたことで、既存環境の構成の性質は設計根拠としても書かないことになった。判断が変わったのは前回の判断が誤っていたからではなく、判定の基準が増えたためである。
