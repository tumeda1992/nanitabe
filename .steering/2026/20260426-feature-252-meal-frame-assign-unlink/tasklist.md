...
- [ ] ユーザーに以下の動作確認を依頼する
  - [ ] ① 枠付き DishCard → アクションパネル → 「枠解除」→ 枠が外れ FrameCard が再出現することを確認
  - [ ] ② FrameCard クリック → 2タブモーダル → 「既存の食事を割り当て」→ 食事を選択して割り当てられることを確認
  - [ ] ③ 食事がない日の空エリア `+` クリック → 3タブモーダル（食事/枠/枠パターン適用）が開くことを確認
