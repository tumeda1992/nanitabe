...
export default ({ onSubmitSucceeded, defaultName = '', defaultDaySlots = [] }: Props) => {
  const { addMealFramePattern } = useMealFramePattern();
  ...
  const updateEntry = (dayIndex: number, entryIndex: number, field: keyof EntryFormData, value: number) => {
    const updated = daySlots.map((slot, i) => {
      if (i !== dayIndex) return slot;
      return {
        ...slot,
