...
const baseProps = {
  ...
  calenderModeChangers: { startMovingDishMode: jest.fn() },
...
describe('<DateCard>', () => {
  describe('表示テスト', () => {
    ...
    it('isDisplayCalenderMode=true のとき + ボタンが表示されること', () => {
      renderWithApollo(<DateCard {...baseProps} isDisplayCalenderMode={true} />);
      expect(screen.getByTestId('dateCard-addMeal')).toBeInTheDocument();
    });

    it('isDisplayCalenderMode=false のとき + ボタンが表示されないこと', () => {
      renderWithApollo(
        <DateCard {...baseProps} isDisplayCalenderMode={false} />,
      );
      expect(screen.queryByTestId('dateCard-addMeal')).not.toBeInTheDocument();
    });

    it('meals が空 かつ isDisplayCalenderMode=true のとき点線の追加エリアが表示されること', () => {
      renderWithApollo(
        <DateCard {...baseProps} meals={[]} isDisplayCalenderMode={true} />,
      );
    ...
    it('meals が存在するとき点線の追加エリアが表示されないこと', () => {
      const meals = [buildMeal()];
      renderWithApollo(
        <DateCard {...baseProps} meals={meals} isDisplayCalenderMode={true} />,
      );
      expect(
  ...
  describe('機能テスト', () => {
    ...
    it('meals が存在するとき CalenderMealIcon が meal の数だけレンダリングされること', () => {
