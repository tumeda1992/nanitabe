...
12. ユーザーから問題なしの確認が取れたら、コミットを提案する
...
# コミットルール
- **MUST: ユーザーの許可なしにコミットしない**
- **MUST: まとめて1コミットにしない**。フェーズ単位・意味的に異なる変更単位で分割してコミットする
  - 例: 「DB マイグレーション」「ドメインモデル」「GraphQL」「フロントエンド」は別コミット
  - フェーズが複数に渡る場合は、フェーズの順序を考慮してコミット順を決める
- ユーザーが一部のみ OK の場合は、OK の範囲だけコミットして残りは待つ
- **コミットは目的でなく手段**。実装内容に認識齟齬がある状態でコミットを急かさない
  - 動作確認の結果に問題があれば、修正を優先してコミット提案は後回しにする
